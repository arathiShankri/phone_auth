...
class PhoneAuthScreenC extends StatefulWidget {
  PhoneAuthScreenC({Key key, this.title}) : super(key: key);

  final String title;

  @override
  _PhoneAuthScreenStateC createState() => _PhoneAuthScreenStateC();
}

class _PhoneAuthScreenStateC extends State<PhoneAuthScreenC> {
...
// This method displays the enter phone num widget or enter sms code widget depending on whether the user had entered the phone number.
  ...
  _sendCode(BuildContext context) async {
    this._authBloc.add(
        AuthModule().get<SendCode>(additionalParameters: {'phoneNumber': phoneNumController.text})); // this event will either send back Authenticated or UnAuthenticated
